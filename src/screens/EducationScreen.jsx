...
const EducationScreen = () => {
    ...
    const updateExercise = async () => {
        const UPDATE_USER_EDUCATION_MUTATION = `
          mutation UpdateUserEducation($userId: String!, $education: String!) {
            updateShinkUser(input: { userId: $userId, education: $education }) {
              userId
              education
            }
          }
        ...
        try {
            const response = await fetch('https://6zdkqxsh5nb57gljtisvt3rxre.appsync-api.ap-south-1.amazonaws.com/graphql', {
                ...
                body: JSON.stringify({
                    query: UPDATE_USER_EDUCATION_MUTATION,
                    variables: { userId: '8', education: selectedOption },
                }),
            });
            const data = await response.json();
            console.log('API Response: ', data);
            ...
            console.error('Error while updating user education: ', error);
        }
    };
        ...
        <View style={styles.container}>
            <LinearGradient start={{ x: 0, y: 0 }}
                ...
                <TouchableOpacity onPress={() => handlePreviousButtonPressed()}
                    ...
                    <Text style={styles.titleTextStyle}>Education</Text>
                </TouchableOpacity>
            </LinearGradient>
            <View style={styles.questionContainer}>
                <Text style={styles.questionTextStyle}>The educational attainment level?</Text>
                <View>
                    <OptionButton label="High School"
                        onPress={() => setSelectedOption('HighSchool')}
                        selected={selectedOption === 'HighSchool'} />
                    <OptionButton label="In College"
                        onPress={() => setSelectedOption('InCollege')}
                        selected={selectedOption === 'InCollege'} />
                    <OptionButton label="Undergraduate"
                        onPress={() => setSelectedOption('undergraduate')}
                        selected={selectedOption === 'undergraduate'} />
                    <OptionButton label="Graduated"
                        onPress={() => setSelectedOption('graduated')}
                        selected={selectedOption === 'graduated'} />
                    <OptionButton label="Postgraduate"
                        onPress={() => setSelectedOption('postegraduate')}
                        selected={selectedOption === 'postgraduate'} />
                </View>
            </View>
            <LinearGradient start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
