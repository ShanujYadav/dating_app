...
const DrinkingScreen = () => {
    ...
    const updateExercise = async () => {
        const UPDATE_USER_DRINKING_MUTATION = `
          mutation UpdateUserDrinking($userId: String!, $drinking: String!) {
            updateShinkUser(input: { userId: $userId, drinking: $drinking }) {
              userId
              drinking
            }
          }
        ...
        try {
            const response = await fetch('https://6zdkqxsh5nb57gljtisvt3rxre.appsync-api.ap-south-1.amazonaws.com/graphql', {
                ...
                body: JSON.stringify({
                    query: UPDATE_USER_DRINKING_MUTATION,
                    variables: { userId: '8', drinking: selectedOption },
                }),
            });
            const data = await response.json();
            console.log('API Response: ', data);
            ...
            console.error('Error while updating user drinking: ', error);
        }
    };
        ...
        <View style={styles.container}>
            <LinearGradient start={{ x: 0, y: 0 }}
                ...
                <TouchableOpacity onPress={() => handlePreviousButtonPressed()}
                    ...
                    <Text style={styles.titleTextStyle}>Drinking</Text>
                </TouchableOpacity>
            </LinearGradient>
            <View style={styles.questionContainer}>
                <Text style={styles.questionTextStyle}>Do you drink alcohol?</Text>
                <View>
