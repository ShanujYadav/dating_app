...
const KidsScreen = () => {
    ...
    const updateExercise = async () => {
        const UPDATE_USER_KIDS_MUTATION = `
          mutation UpdateUserKids($userId: String!, $kids: String!) {
            updateShinkUser(input: { userId: $userId, kids: $kids }) {
              userId
              kids
            }
          }
        ...
        try {
            const response = await fetch('https://6zdkqxsh5nb57gljtisvt3rxre.appsync-api.ap-south-1.amazonaws.com/graphql', {
                ...
                body: JSON.stringify({
                    query: UPDATE_USER_KIDS_MUTATION,
                    variables: { userId: '8', kids: selectedOption },
                }),
            });
            const data = await response.json();
            console.log('API Response: ', data);
            ...
            console.error('Error while updating user kids: ', error);
        }
    };
        ...
        <View style={styles.container}>
            <LinearGradient start={{ x: 0, y: 0 }}
                ...
                <TouchableOpacity onPress={() => handlePreviousButtonPressed()}
                    ...
                    <Text style={styles.titleTextStyle}>Kids</Text>
                </TouchableOpacity>
            </LinearGradient>
            <View style={styles.questionContainer}>
                <Text style={styles.questionTextStyle}>What do you prefer?</Text>
                <View>
                    <OptionButton label="Not for now"
                        onPress={() => setSelectedOption('notForNow')}
                        selected={selectedOption === 'notForNow'} />
                    <OptionButton label="Maybe in future"
                        onPress={() => setSelectedOption('maybeInFuture')}
                        selected={selectedOption === 'maybeInFuture'} />
                    <OptionButton label="Yes we have"
                        onPress={() => setSelectedOption('yesWeHave')}
                        selected={selectedOption === 'yesWeHave'} />
