...
const ExerciseScreen = () => {
    ...
    const updateExercise = async () => {
        const UPDATE_USER_EXERCISE_MUTATION = `
          mutation UpdateUserExercise($userId: String!, $exercise: String!) {
            updateShinkUser(input: { userId: $userId, exercise: $exercise }) {
              userId
              exercise
            }
          }
        ...
        try {
            const response = await fetch('https://6zdkqxsh5nb57gljtisvt3rxre.appsync-api.ap-south-1.amazonaws.com/graphql', {
                ...
                body: JSON.stringify({
                    query: UPDATE_USER_EXERCISE_MUTATION,
                    variables: { userId: '8', exercise: selectedOption },
                }),
            });
            const data = await response.json();
            console.log('API Response: ', data);
            ...
            console.error('Error while updating user exercise: ', error);
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.questionContainer}>
                <Text style={styles.questionTextStyle}>Do you do any physical activity?</Text>
                <View>
                    <OptionButton label="Yes"
                        onPress={() => setSelectedOption('Yes')}
                        selected={selectedOption === 'Yes'} />
                    <OptionButton label="Maybe"
                        onPress={() => setSelectedOption('Maybe')}
                        selected={selectedOption === 'Maybe'} />
                    <OptionButton label="Nope"
                        onPress={() => setSelectedOption('Nope')}
                        selected={selectedOption === 'Nope'} />
                </View>
            </View>
            <LinearGradient start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
